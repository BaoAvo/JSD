...
public class LibraryManProg extends WindowAdapter {
    private JFrame mainGui;
    private JPanel southPanel, centrePanel;
    ...
    private void createGUI() {
        mainGui = new JFrame("Library Management");
        mainGui.addWindowListener(this);
        //Create a menu bar
        ...
        //The centre panels
        centrePanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 30, 20));
        Image backgroundImage = null;
        try{
            backgroundImage = ImageIO.read(getClass().getResource("library_background.jpg"));
        }catch(Exception e){
            e.printStackTrace();
        }
        backgroundImage = backgroundImage.getScaledInstance(500, 300, java.awt.Image.SCALE_SMOOTH);

        JLabel imageLabel = new JLabel(new ImageIcon(backgroundImage));
        centrePanel.add(imageLabel);
        mainGui.add(centrePanel, BorderLayout.CENTER);

        //The south panels
        southPanel = new JPanel();
        JLabel copyrightFooter = new JLabel("©2023 – Nguyen Van Quan design. All rights reserved.", JLabel.CENTER);
        southPanel.add(copyrightFooter);
        mainGui.add(southPanel, BorderLayout.SOUTH);
    ...
    public static void main(String[] args) {
        LibraryManProg main = new LibraryManProg();
        main.display();
